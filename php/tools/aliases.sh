...
alias c='clear'
alias art=artisan
alias sf='./vendor/bin/console'
alias c='composer'

alias codecept='vendor/bin/codecept'
alias phpspec='vendor/bin/phpspec'
alias pint='vendor/bin/pint'
alias serve=serve-booker
...
function serve-booker() {
    if [[ "$1" && "$2" ]]
    then
        sudo bash /vagrant/scripts/create-certificate.sh "$1"
        sudo dos2unix /vagrant/scripts/site-types/booker.sh
        sudo bash /vagrant/scripts/site-types/booker.sh "$1" "$2" 80 443 "${3:-8.1}"
    else
        echo "Error: missing required parameters."
        echo "Usage: "
